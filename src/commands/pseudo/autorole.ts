import { ActionRowBuilder, GuildMember, RoleSelectMenuBuilder, StringSelectMenuBuilder } from "discord.js";
...
const settings = async (interaction: any) => {
    const autorole_system = await DatabaseConnection.manager.findOne(Autorole, { where: { from_guild: { gid: interaction.guild.id } } });
    if (!autorole_system) {
        const new_autorole = new Autorole();
        new_autorole.from_guild = await DatabaseConnection.manager.findOne(Guilds, { where: { gid: interaction.guild.id } });
        new_autorole.latest_action_from_user = await DatabaseConnection.manager.findOne(Users, { where: { uid: interaction.user.id } });
    ...
    const role_select_menu = new RoleSelectMenuBuilder().setCustomId('settings:autorole:21').setPlaceholder('Select a role');

    const createMenuOptions = () => [
        { label: `${status} Autorole System`, description: `${status} the autorole system`, value: 'settings:autorole:1' },
        { label: 'Change Autorole Role', description: 'Edit the role that is given to new users', value: 'settings:autorole:2' },
