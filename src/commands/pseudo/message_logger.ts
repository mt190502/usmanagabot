    ...
    switch (menu_path) {
        ...
        case '3':
            await interaction.update({
                ...
                components: [new ActionRowBuilder().addComponents(channel_select_menu.setCustomId('settings:logger:31').setMinValues(1).setMaxValues(10).setDefaultChannels(logger.ignored_channels?.length > 0 ? logger.ignored_channels.map((channel) => channel.toString()) : []))]
            });
            break;
        case '31':
